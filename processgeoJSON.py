...
for feature in line_features:
    ...
    # Determine accessibility (can extend this to use properties)
    accessible = True
...
for feature in polygon_features:
    ...
    if feature_name:
        ...
        # Process each coordinate pair in outer ring
        for coord in outer_ring:
            ...
            # Connect this node to nearest path node (excluding buildings)
            connect_node_to_nearest_node(
                node_id, lat, lon, nodes, edges, accessible, exclude_types={'building'}
            )

        # Create edges between consecutive nodes (building perimeter)
...
# Process Point features (entrances, waypoints, others)
for feature in point_features:
    properties = feature.get('properties', {})
    feature_name = properties.get('name', '')
    feature_type = properties.get('type', 'point')
    ...
    if existing_node_id is not None:
        ...
        if not nodes[node_id]['name']:
            nodes[node_id]['name'] = feature_name
        if not nodes[node_id]['type']:
            nodes[node_id]['type'] = feature_type
    else:
        node_id = node_id_counter
        node_id_counter += 1
        nodes[node_id] = {
    ...
    # Optionally connect points to nearest path nodes
    connect_node_to_nearest_node(
        node_id, lat, lon, nodes, edges, True, exclude_types={'building'}
    )
